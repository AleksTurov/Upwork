...
def handle_message(message):
    ...
    try:
        ...
        prompt = message.text
        completion = openai.Completion.create(engine=engine, prompt=prompt, temperature=1.5, max_tokens=4000)

        # Проверяем наличие ожидаемого поля в ответе от OpenAI
        if 'choices' in completion and len(completion.choices) > 1 and 'text' in completion.choices[0]:
            bot.send_message(chat_id=group_id, text=completion.choices[1]['text'])
        else:
